 ...
 */
public class SpoutObserverBolt extends WindowedBolt {
	private transient OutputCollector collector;
	private transient Aggregator aggregator;
	private long startTime;
	private long totalCount;

	public SpoutObserverBolt(long tickFrequencyInSeconds) {
	...
	private void emitFirstTime() {
		...
		message.addField("EVENT_TIME", startTime);
		message.addField("EVENT_TIME_FORMATTED", DKGUtils.formattedTime(startTime));
		message.addField("TOTAL_TIME_CONSUMPTION", 0);
	...
	public void emitCurrentWindowAndAdvance() {
		...
		emitOngoingTimeConsumption(timestamp);
	}

	private void emitOngoingTimeConsumption(long timestamp) {
